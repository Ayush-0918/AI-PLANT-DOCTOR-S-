      ...
      <AnimatePresence mode="wait">
        ...
        {step !== 'splash' && (
          ...
          >
            ...
            {/* ────── FARMER BASICS & SIMPLE AUTHENTICATION ────── */}
            {step === 'farmer' && (
              <div className="flex h-full flex-col px-6 pb-6 pt-6 bg-white">
                ...
                <div className="mb-4 shrink-0">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 text-xs font-bold border border-emerald-200 hover:bg-emerald-100 transition-colors"
                    >
                      <Stars size={14} />
                      <span>{language === 'हिंदी' ? '⚡ 1-Click Demo' : '⚡ 1-Click Demo'}</span>
                    </button>
                  </div>
                  <p className="mt-1.5 text-sm text-slate-600">{S.farmerSub}</p>
                  ...
                  <div className="mt-3">
                    <button
                      ...
                      disabled={authLoading}
                      className="flex w-full items-center justify-center gap-2.5 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 font-semibold text-slate-700 hover:bg-slate-100 active:scale-98 transition-all text-sm"
                    >
                      ...
                      <span>{language === 'हिंदी' ? 'Continue with Google (गूगल साइन-इन)' : 'Continue with Google'}</span>
                    </button>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto hide-scrollbar space-y-4 pb-4">
                  {authError && (
                    <div className="rounded-xl bg-rose-50 border border-rose-200 p-3 text-xs font-semibold text-rose-600 text-center">
                      {authError}
                    </div>
                  )}

                  <form onSubmit={handleFarmerSubmit} id="farmer-form" className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g. Kishan Kumar / किशन कुमार"
                        className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5 text-slate-900 focus:border-blue-600 focus:bg-white focus:outline-none transition-all"
                      />
                    </div>
                        ...
                        return (
                          <button
                            ...
                            className={`flex w-full items-center justify-between rounded-xl border px-4 py-3.5 text-left transition-colors ${
                              active ? 'border-blue-600 bg-blue-50/50 text-blue-900 font-semibold' : 'border-slate-200 bg-white text-slate-700'
                            }`}
                          >
                            <span>{entry}</span>
                ...
                <div className="shrink-0 pt-3 bg-white">
                  <button
                    ...
                    form="farmer-form"
                    disabled={authLoading}
                    className="w-full rounded-full bg-blue-600 px-5 py-4 font-bold text-white hover:bg-blue-700 active:scale-95 transition-transform shadow-md shadow-blue-600/20 text-center"
                  >
                    {authLoading ? (language === 'हिंदी' ? 'सेव हो रहा है...' : 'Saving...') : S.saveContinue}
                  </button>
                </div>
              </div>
            )}
